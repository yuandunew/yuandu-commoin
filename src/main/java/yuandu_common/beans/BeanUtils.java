...
public class BeanUtils {
    ...
    private static <T> T doCopy(Object src, T dest, boolean setDefaultValForNull, String... includeFields) throws BeanConverterException {
        ...
        try {
            ...
            for (String key : keys) {
                ...
                //是否指定了copy字段
                if (includeFieldList != null && !includeFieldList.contains(name)) continue;
    ...
    public static Method getWriteMethod(Class<?> clazz, String fieldName) {
        try {
            ...
            if (!propertyDescriptorCache.containsKey(canonicalName)) return null;
    ...
    private synchronized static Map<String, PropertyDescriptor> doCacheClass(Class<?> clazz, String canonicalName)
            throws IntrospectionException {
        if (propertyDescriptorCache.containsKey(canonicalName)) return propertyDescriptorCache.get(canonicalName);
    ...
    public static Object getPropertyValue(Object obj, String fieldName) {
        try {
            ...
            if (propertyDescriptorCache.containsKey(canonicalName)) {
                map = propertyDescriptorCache.get(canonicalName);
                PropertyDescriptor fd = map.get(fieldName);
                if (fd != null) {
                    return fd.getReadMethod().invoke(obj);
                }
            }

            Field[] fields = obj.getClass().getDeclaredFields();
            for (Field f : fields) {
                PropertyDescriptor pd = getPropertyDescriptor(obj.getClass(), f.getName());
                map.put(f.getName(), pd);
            }
            propertyDescriptorCache.put(canonicalName, map);
            PropertyDescriptor fd = map.get(fieldName);
            if (fd != null) {
     ...
     * @return
     */
    public static Class<?> getPropertyType(Class<? extends Object> clazz, String fieldName) {
        try {
            Map<String, PropertyDescriptor> map = new ConcurrentHashMap<String, PropertyDescriptor>();
            String canonicalName = clazz.getCanonicalName();
            if (propertyDescriptorCache.containsKey(canonicalName)) {
                ...
                PropertyDescriptor fd = map.get(fieldName);
                if (fd != null) {
                    return fd.getPropertyType();
                }
            }

            Field[] fields = clazz.getDeclaredFields();
            for (Field f : fields) {
                PropertyDescriptor pd = getPropertyDescriptor(clazz, f.getName());
                map.put(f.getName(), pd);
            }
            propertyDescriptorCache.put(canonicalName, map);
            PropertyDescriptor fd = map.get(fieldName);
